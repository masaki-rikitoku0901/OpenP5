...
class SingleRunner:
    def parse_runner_args(parser):
        ...
        parser.add_argument(
            "--load", type=int, default=0, help="load model from model path or not."
        )
        parser.add_argument(
            "--random_initialize",
            type=int,
        ...
        parser.add_argument(
            ...
            type=int,
            default=0,
            help="use validation loss to select models",
        )
        parser.add_argument(
            "--test_before_train",
            type=int,
            default=1,
            help="whether test before training",
        )
        parser.add_argument(
            "--test_filtered",
            type=int,
    ...
    def __init__(self, model, tokenizer, train_loader, valid_loader, device, args):
        ...
        self.device = device
        self.args = args
        self.regenerate_candidate = (
            "candidate_items" in self.train_loader.dataset.datasets[0].info
        )
    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            logging.info(
                f"The average training loss for epoch {epoch+1} is {train_epoch_loss}"
            )
            ...
            if self.valid_select > 0:
                ...
                with torch.no_grad():
                    for batch in tqdm(self.valid_loader):
                        ...
                        loss = loss.view(B, L) * lm_mask
                        loss = (
                            loss.sum(dim=1) / lm_mask.sum(dim=1).clamp(min=1)
                        ).mean()
                    ...
                    logging.info(
                        f"The average valid loss for epoch {epoch+1} is {valid_epoch_loss}"
                    )

                    if valid_epoch_loss == min(valid_losses):
                        logging.info("The minimal validation loss so far.")
                        best_epoch = epoch + 1
                        utils.save_model(self.model, self.args.model_path)
                        logging.info(
                            f"Save the current model to {self.args.model_path}"
                        )
    ...
    def create_optimizer_and_scheduler(self):
        ...
        total_steps = (
            batch_per_epoch // self.args.gradient_accumulation_steps * self.args.epochs
        )
    ...
    def test_dataset_task_filtered_batch(self, testloader):
        logging.info(
            f"testing filtered {testloader.dataset.dataset} dataset on {testloader.dataset.task} task"
        )
        test_total = 0
        with torch.no_grad():
            ...
            candidate_trie = gt.Trie(
                [
                    ...
                    + self.tokenizer.encode(
                        f"{testloader.dataset.dataset} item_{candidate}"
                    )
            ...
            for batch in tqdm(testloader):
                ...
                prediction = self.model.module.generate(
                    ...
                    + testloader.dataset.max_positive,
                    output_scores=True,
    ...
    def test_dataset_task_filtered(self, testloader):
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(testloader):
                ...
                positive = testloader.dataset.positive[
                    testloader.dataset.id2user[user_idx]
                ]
    ...
    def test_dataset_task(self, testloader):
        logging.info(
            f"testing {testloader.dataset.dataset} dataset on {testloader.dataset.task} task"
        )
        test_total = 0
        with torch.no_grad():
            candidates = testloader.dataset.all_items
